function fetchAllTags() {
    ...
    fetch('https://innovision-server.iphigeniebera.repl.co/alltags')
...
function displayImagesByTag(tag) {
    ...
    fetch(`https://innovision-server.iphigeniebera.repl.co/tag/${tag}`)
        .then(response => response.json())
        .then(res => {
            var imageUrls = res.data;
            imageUrls.forEach(image => {
                //https://photoapp-iphigenie-cs310.s3.us-east-2.amazonaws.com/t2.jpg
                var imageUrl = `https://photoapp-iphigenie-cs310.s3.us-east-2.amazonaws.com/${image}`;
...
function uploadImage(file) {
    ...
    const url = 'https://innovision-server.iphigeniebera.repl.co/upload';
...
function displayextracted(filename, extractedtags) {
    ...
    var imageUrl = `https://photoapp-iphigenie-cs310.s3.us-east-2.amazonaws.com/${filename}`;
    console.log(imageUrl);
    const imgElement = document.createElement('img');
    imgElement.src = imageUrl;
    imgElement.classList.add('image');
